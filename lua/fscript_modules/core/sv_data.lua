...
function FScript.LoadCharacter(ply, characterNumber)
	...
	local CharacterData = file.Read("fscript_data/players/" .. ply:SteamID64() .. "/" .. characterNumber .. ".json")
	if not FScript.IsValidString(CharacterData) then
		FScript.SendNotification(ply, 1, FScript.Lang.InvalidData)
		return
	end

	CharacterData = util.JSONToTable(CharacterData)
	...
	if DarkRP then
		local CharacterJob = CharacterData["Job"]
		...
		local CharacterMoney = CharacterData["Money"]
		if CharacterMoney then
			ply:setDarkRPVar("money", tonumber(CharacterMoney))
		end

		if not DarkRP.disabledDefaults["modules"]["hungermod"] then
			local CharacterHunger = CharacterData["Hunger"]
		...
		local WantedCharacter = CharacterData["Wanted"]
		if WantedCharacter then
			ply:setDarkRPVar("wanted", true)
			ply:setDarkRPVar("wantedReason", CharacterData["WantedReason"])
		end

		local ArrestedCharacter = CharacterData["Arrested"]
	...
	if itemstore then
		CharacterData["Inventory"] = util.JSONToTable(CharacterData["Inventory"])
		CharacterData["Bank"] = util.JSONToTable(CharacterData["Bank"])

		for k, v in pairs(CharacterData["Inventory"]) do
			ply.Inventory:SetItem(k, itemstore.Item(v.Class, v.Data))
		end

		for k, v in pairs(CharacterData["Bank"]) do
			ply.Bank:SetItem(k, itemstore.Item(v.Class, v.Data))
		end
	end

	local SaveMap = CharacterData["Map"]
	if SaveMap and SaveMap == game.GetMap() then
		local CharacterPosition = CharacterData["Position"]
		if CharacterPosition then
			ply:SetPos(Vector(CharacterPosition))
		end

		local CharacterAngle = CharacterData["Angle"]
		if CharacterAngle then
			ply:SetEyeAngles(Angle(CharacterAngle))
		end
	end

	local CharacterHealth = CharacterData["Health"]
	if CharacterHealth then
		ply:SetHealth(tonumber(CharacterHealth))
	end

	local CharacterArmor = CharacterData["Armor"]
	if CharacterArmor then
		ply:SetArmor(tonumber(CharacterArmor))
	end

	local CharacterModel = CharacterData["Model"]
	if CharacterModel and util.IsValidModel(CharacterModel) then
		ply:SetModel(CharacterModel)
	end

	local CharacterSkin = CharacterData["Skin"]
	...
	local CharacterDescription = CharacterData["Description"]
	if CharacterDescription then
		ply:SetCharacterDescription(CharacterDescription)
	end

	local CharacterWeapons = CharacterData["Weapons"]
	...
	local CharacterAmmos = CharacterData["Ammos"]
	...
	local CharacterBodyGroups = CharacterData["BodyGroups"]
	...
	hook.Run("FScript.OnCharacterLoaded", ply, CharacterData)
end